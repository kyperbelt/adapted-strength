export default function LabeledInputField({ type, placeholder, className, id, name, required, pattern, defaultValue, ...props }) {
    return (
        <div className={`relative ${className}`}>
            <input type={type} id={id} name={name ?? id} className={`block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-primary rounded-lg border-2 border-primary-dark appearance-none focus:outline-none focus:ring-0 focus:border-secondary-light peer`} placeholder=" "
