...
import CheckoutPage from "../PaymentCheckout";
...
function onConnected() {
  ...
  // register the connected user
  stompClient.send(
    "/app/chatUser.addUser",
    {},
    JSON.stringify({ email: nickname, fullName: fullname })
  );
}
...
function MyButton({ text, ...props }) {
  return (
    <button
      type="button"
      className="bg-red-500 rounded-lg p-4 focus:bg-red-900 text-white hover:text-sky-50"
      {...props}
    >
      {" "}
      {text}
