...
public class ControllerHandlerManager {
    ...
    private static <T> ControllerHandler buildBean(T serviceObject, String methodName, Class<?>... argTypes)
            throws NoSuchMethodException, SecurityException {
        // Check Preconditions
        if (argTypes.length > 3)
            throw new IllegalArgumentException("argTypes can have at most 3 parameters");
        for (Class<?> clazz : argTypes) {
            if (clazz == Map.class)
                ;
            else if (clazz == RequestEntity.class)
                ;
            else if (clazz == HttpServletRequest.class)
                ;
            else
                throw new IllegalArgumentException("argTypes must one of Map, RequestEntity, or ServletRequest");
        }
